...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		if (!isMobile()) {
			...
			$(".slowed-pin .slowed-text a").on('mouseenter', function() {	
				var $this = $(this);			
			...
			$(".slowed-pin .slowed-text a").on('mouseleave', function() {					
				...
				if ($(window).width() > 479) {					
					...
					//Fixed header - toggle based on scroll direction
...
//				  animation: gsap.fromTo('.resizeMe', {scale: 1 }, {scale: 10}),
				...
				const headerTop = gsap.utils.toArray('header');
				headerTop.forEach((headerT) => {
				  gsap.from(headerT, {
				    scrollTrigger: {
				      start: 'top bottom',
				      end: 'bottom top',
				      trigger: headerT,
				      toggleClass: 'headerTop'
				    }
				  });
				});								
				
				
				//Fixed Asset Menu - toggle based on scroll direction
												
				const assetView = gsap.from('.asset-view-nav', { 
				  yPercent: -100,
				  paused: true,
//				  animation: gsap.fromTo('.resizeMe', {scale: 1 }, {scale: 10}),
				  duration: 0.4
				...
				ScrollTrigger.create({
				  ...
				  onUpdate: (self) => {
				    self.direction === -1 ? assetView.play() : assetView.reverse()
				  }
				});		
				
				const assetViewTop = gsap.utils.toArray('.asset-view-nav');
				assetViewTop.forEach((headerT) => {
				  gsap.from(headerT, {
				    scrollTrigger: {
				      start: 'top bottom',
				      end: 'bottom top',
				      trigger: headerT,
				      toggleClass: 'assetViewTop'
				    }
				  });
				});				
				// Moving Gallery		
				...
				const roll2 = roll(".marquee-text.bw", {duration: 15}, true);
				const roll3 = roll(".marquee-title .hero-title", {duration: 10});
				const roll4 = roll(".marquee-title .next-hero-title", {duration: 20});
	...
	// 	Page and Project Navigation
		...
		if ($(".content-row").hasClass("light-section")) {
			$(".light-section").each(function(i) {				
				$(this).wrap( "<div class='light-section-wrapper'><div class='light-section-container content-full-width'></div></div>" );
		...
		if ($(".content-row").hasClass("dark-section")) {
			$(".dark-section").each(function(i) {				
				$(this).wrap( "<div class='dark-section-wrapper'><div class='dark-section-container content-full-width'></div></div>" );			
		...
		$('.content-carousel').each(function(){
			...
			tlCarousel.set($(".content-carousel .swiper-slide"), {x: 400, opacity:0});
